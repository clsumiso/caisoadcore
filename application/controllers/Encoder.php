...
class Encoder extends CI_Controller {
	...
	{
		$semester = 8;
		$student_data = $this->encoder->get_student_info($_POST['studid'], 8);
		$name = '---';
		$course = $_POST['course'];
		$section = $_POST['section'];
		$htmlData = '';
		...
		{
            $approve = count($this->encoder->get_enrolled($subject->schedid, 'approved', 8));
            $decline = count($this->encoder->get_enrolled($subject->schedid, 'declined', 8));
            $decline = count($this->encoder->get_enrolled($subject->schedid, '', 8));
			$htmlData .= '<tr>';
				$htmlData .= '<td>'.($i++).'</td>';
				$htmlData .= '<td>
    ...
    public function showSubjects()
    {
        ...
        $decline = 0;
        $pending = 0;
        $subject_data = $this->encoder->getClassScheduleByFilter($filter, 8);
	...
	{
		$enrollData = $this->encoder->get_enrollment($_POST['studid'], 8);
		$htmlData = '';
		$i = 1;
